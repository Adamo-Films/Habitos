...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function resetMinigameState() {
    ...
    updateMinigameHud();
    updateMinigameOverlay('Pressione Start', 'Desvie, segure o tempo e evolua até o nível 5. Cada novo nível melhora seu fogo, mas qualquer acerto remove o último poder — o jogo acaba se você for atingido no nível 1.');
  }
  ...
  function spawnEnemy(forcedType) {
    ...
    const size = (26 + Math.random() * 14) * 3;
    const base = {
      x,
  ...
  function triggerPop(type) {
    ...
    const duration = type === 'evolve' ? 0.65 : 0.5;
  ...
  function renderMinigame() {
    ...
    minigameState.pops.forEach((pop) => {
      ...
      const fade = 1 - t;
      const baseRadius = pop.type === 'evolve' ? 38 : 30;
      const maxRadius = pop.type === 'evolve' ? 120 : 90;
      const radius = baseRadius + easeOut(t) * (maxRadius - baseRadius);
      const innerRadius = radius * 0.55;
      ctx.save();
      ctx.globalAlpha = fade;
      ctx.strokeStyle = pop.type === 'evolve' ? 'rgba(81, 255, 231, 0.7)' : 'rgba(255, 114, 182, 0.75)';
      ctx.lineWidth = pop.type === 'evolve' ? 6 - t * 3 : 5 - t * 2;
      ctx.beginPath();
      ctx.arc(pop.x, pop.y, radius, 0, Math.PI * 2);
      ctx.stroke();
      ctx.strokeStyle = pop.type === 'evolve' ? 'rgba(255, 227, 121, 0.65)' : 'rgba(255, 186, 186, 0.7)';
      ctx.lineWidth = 2.5;
      ctx.beginPath();
