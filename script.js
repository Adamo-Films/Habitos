    ...
    switch (type) {
      case 'slicer':
        minigameState.enemies.push({
          ...
          vx: (Math.random() < 0.5 ? -1 : 1) * (190 + Math.random() * 60),
          hp: 2,
          wobble: 0,
          dashTimer: 1 + Math.random(),
          type,
          value: 2
        });
        break;
      case 'orbiter':
        minigameState.enemies.push({
          ...
          wobble: 0.9 + Math.random() * 1.3,
          phase: Math.random() * Math.PI * 2,
          hp: 2,
          fireCooldown: 1 + Math.random() * 0.6,
          type,
          value: 3
        });
        break;
    ...
    minigameState.enemies.forEach((enemy) => {
      switch (enemy.type) {
        case 'slicer':
          ...
          const dashBoost = enemy.dashTimer <= 0 ? 1.9 : 1;
          enemy.y += enemy.vy * dashBoost * delta;
          enemy.x += enemy.vx * dashBoost * delta;
          if (enemy.dashTimer <= 0) {
            enemy.dashTimer = 1.2 - Math.min(0.6, minigameState.difficulty * 0.08);
            enemy.vx *= -1;
          }
        ...
        case 'orbiter':
          ...
          enemy.x += Math.sin(enemy.phase) * 42 * delta + enemy.vx * delta;
          enemy.fireCooldown -= delta;
          if (enemy.fireCooldown <= 0) {
            minigameState.enemyBullets.push({
              x: enemy.x,
              y: enemy.y + enemy.size * 0.3,
              vx: Math.sin(enemy.phase) * 60,
              vy: 220 + minigameState.difficulty * 40,
              size: 6
            });
          ...
          if (enemy.fireCooldown <= 0) {
            ...
            spread.forEach((angle) => {
              minigameState.enemyBullets.push({
                ...
                vy: 240 + minigameState.difficulty * 50,
                size: 6
              });
            });
