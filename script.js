...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function renderWeightLog() {
    ...
    const positionOverlay = (coord) => {
      ...
      const xPx = (coord.x / viewWidth) * svgRect.width + offsetX;
      const yPx = (coord.y / viewHeight) * svgRect.height + offsetY;
      ...
      const padding = 12;
      let desiredX = xPx + 14;
      if (desiredX + tooltipWidth + padding > surfaceRect.width) {
        desiredX = xPx - tooltipWidth - 14;
      }
      desiredX = Math.min(surfaceRect.width - tooltipWidth - padding, Math.max(padding, desiredX));

      let desiredY = yPx - tooltipHeight - 16;
      if (desiredY < padding) {
        desiredY = yPx + 16;
      }
      desiredY = Math.min(surfaceRect.height - tooltipHeight - padding, Math.max(padding, desiredY));
